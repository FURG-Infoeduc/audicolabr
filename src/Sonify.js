...
const techniques = {
  ...
  falasinte: {
    ...
    addition(e) { // {user: int, dir: {nodes: [], pan: int}}
      ...
      const msg = e.distance && e.distance > 0 ?
        `${username} incluiu uma tabela ${e.distance} links à ${direction}` :
        //`${username} incluiu uma tabela à ${direction}` :
        `${username} incluiu essa tabela`;
      const utter = new SpeechSynthesisUtterance(msg);
      utter.onstart = () => { console.log(Date.now()); }
      utter.onend = () => { console.log(Date.now()); }
  ...
  falagrava: {
    ...
    addition(e) {
      ...
      audio.onplay = () => { console.log(Date.now()); }
      audio.onended = () => { console.log(Date.now()); }
      audio.play().catch((e) => {
